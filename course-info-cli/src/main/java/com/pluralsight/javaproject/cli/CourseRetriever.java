...
//        List<PluralsightCourse> coursesToStore = courseRetrievalService.getCoursesFor(authorId);
    ...
    //after introducing spf4j-api dependency
    //setting up a logger

    //working with the http webclient API
    //creating class courseretrieval service

    //H2 is a database that is an embedded SQL database in java meaning that we dont have to install a seperate database to use that
    ...
    //JDBC= java database connectivity; a standardized java API to connect to any SQL databases
